...
class FirestoreHelper {
  ...
  Future<DocumentSnapshot> get userDocumentSnapshot async {
    final queryResult =
        await favoritesCollection.where('udid', isEqualTo: udid).getDocuments();
  ...
  void syncDatabase(Set<String> favorites) async {
    ...
    if (delta.length > 0 ||
      ...
      delta.forEach((docPath, delta) => batch
          .updateData(statsDocument, {docPath: FieldValue.increment(delta)}));
      await batch.commit();
    }
  }
